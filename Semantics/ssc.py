...
def ssc(keypoints, num_ret_points, tolerance, cols, rows):
    ...
    return selected_keypoints, result_list


def ssc_np_index(kps_arr, num_ret_points, tolerance, cols, rows):
    kps_len = kps_arr.shape[1]
    ...
    low = math.floor(math.sqrt(kps_len/ num_ret_points))
    ...
    while not complete:
        ...
        for i in range(kps_len):
            row = int(
                # math.floor(keypoints[i].pt[1] / c)
                math.floor(kps_arr[1, i]/c)
            )  # get position of the cell current point is located at
            col = int(
                math.floor(kps_arr[0, i]/c)
                # math.floor(keypoints[i].pt[0] / c)
            )
            if not covered_vec[row][col]:  # if the cell is not covered
                result.append(i)
                # get range which current radius is covering
